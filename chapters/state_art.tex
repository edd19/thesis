...
IoT is still an emerging technology and need to resolve some challenges \cite{website:iot_challenge}. This section will be the occasion to discuss about such challenges that IoT is confronted to.
...
\todo[inline]{Reference it via bibliography}
...
\todo[inline]{explain 6lowpan and IEEE 802.15.4 and RPL (reference papers)}
...
Our solution differs from Octopus as we want to use existing protocols so as to integrate easily existent infrastructures. Indeed, Octopus has its own packet format, meaning that monitoring infrastructure need to be modified as to accept it. An other difference lies in that we focus mainly on information on the internal traffic whereas Octopus is a data visualization tool. It is used to show with a GUI interface the values reported by the sensor nodes as temperature, humidity, light, etc. 
