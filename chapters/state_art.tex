...
\item Rime\cite{dunkels2007rime} stack which is an alternative to the two mentioned above.
\end{itemize}
...
This chapter will be the occasion to present what has been done in the field of monitoring tools for networks. The first section will be about the traditional networks and the second one will focus on the IoT networks.
...
The data that SensMap feed on needs to be stored in the Xively public cloud \cite{website:xively}. This means that the sensor devices need to send their values to Xively. Then, SensMap will have access to them via the \acrshort{rest} api offered by Xively.
